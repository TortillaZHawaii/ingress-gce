...
// TestL4Namer verifies that all L4 resource names are of the ed length and format.
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			got := names{
				...
				NetLbFRName:       namer.L4NetLBForwardingRule(tc.namespace, tc.name, strings.ToLower(tc.proto), uint(netLbFRNumber)),
				IPv6FRName:        namer.L4IPv6ForwardingRule(tc.namespace, tc.name, strings.ToLower(tc.proto)),
				NetLbIPv6FRName:   namer.L4NetLBIPv6ForwardingRule(tc.namespace, tc.name, strings.ToLower(tc.proto), uint(netLbFRNumber)),
